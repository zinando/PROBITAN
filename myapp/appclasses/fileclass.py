...
class FILEHANDLER:
    ...
    def get_event_duration_in_months(self, data: list, date_format: str):
        ...
        Returns a dict of month name and number for all the months covered,
        sorted in ascending order of months.
        """
        start_date = []
    ...
    def get_list_of_event_types(self, events: list):
        ...
        event_types = []  # types e.g cil, rls etc
        cil_event_locations = []  # e.g dosing, fb, machine frame
        planned_events = []  # eg dosing, manufacturer splice removal
        unplanned_events = []  # eg heating element replacement

        for x in events:
            if x["class"]:
                if x["class"].split()[0].lower() == "planned":
                    if x["type"]:
                        if x["type"].lower() not in event_types:
                            event_types.append(x["type"].lower())
                        if x["location"]:
                            if x["location"].lower() not in planned_events:
                                planned_events.append(x["location"].lower())
                        if x["type"].lower() == "cil":
                            if x["location"]:
                                if x["location"].lower() not in cil_event_locations:
                                    cil_event_locations.append(x["location"].lower())
                    ...
                    if x["type"]:
                        if x["type"].lower() not in event_types:
                            event_types.append(x["type"].lower())
                        if x["location"]:
                            if x["location"].lower() not in unplanned_events:
                                unplanned_events.append(x["location"].lower())

        return event_types, planned_events, unplanned_events, cil_event_locations

    def compute_time_for_monthly_events(self, monthly_events: list):
        """Computes the total time each event occurred in each month"""
        excluded_events = ["blank"]  # events to be excluded from the analysis
        ...
        event_types, planned_events, unplanned_events, event_locations = self.get_list_of_event_types(all_events)
        event_list = [x for x in (planned_events + unplanned_events) if x not in excluded_events]
        ...
        for item in monthly_events:
            ...
            for i in event_list:# event_types:
                formated = func.enslave_strings(i)
                event_type_time[formated] = 0

            mr['month'] = item['month']

            for x in [x for x in item['events'] if x.get('location') not in excluded_events]:
                ...
                if event_class == 'planned':
                    ...
                    formated = func.enslave_strings(x["location"])
    ...
    def fill_content(self, data: list):
        ...
        for x in data:
            if not x["class"] or x["class"] == '':  # class is planned or unplanned
                x["class"] = "blank"
            if not x["type"] or x["type"] == '':  # type is cil, tbm, rls
                x["type"] = "blank"
            if not x["location"] or x["location"] == '':  # location is workpoints
                x["location"] = "blank"
