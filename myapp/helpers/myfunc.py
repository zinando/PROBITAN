...
def get_date_format(data:list):        
    """Decodes the format of a date/datetime literal from a list of datetimes of unknown formats and converts it to a datetime object"""
        ...
        if find_prime_date(x):
            ...
            if len(check) > 1:
                ...
                if len(datetyme.split(".")) > 1:
                    #it has microseconds
                    possible_formats = ["%Y-%m-%d %H:%M:%S.%f","%Y-%d-%m %H:%M:%S.%f","%d-%m-%Y %H:%M:%S.%f","%m-%d-%Y %H:%M:%S.%f","%Y/%m/%d %H:%M:%S.%f","%Y/%d/%m %H:%M:%S.%f","%d/%m/%Y %H:%M:%S.%f","%m/%d/%Y %H:%M:%S.%f"]
                else:
                    #no microseconds %H:%M:%S.%f
                    possible_formats = ["%Y-%m-%d %H:%M:%S","%Y-%d-%m %H:%M:%S","%d-%m-%Y %H:%M:%S","%m-%d-%Y %H:%M:%S","%Y/%m/%d %H:%M:%S","%Y/%d/%m %H:%M:%S","%d/%m/%Y %H:%M:%S","%m/%d/%Y %H:%M:%S"]    
            else:
                #it is just a date        
                possible_formats = ["%Y-%m-%d","%Y-%d-%m","%d-%m-%Y","%m-%d-%Y","%Y/%m/%d","%Y/%d/%m","%d/%m/%Y","%m/%d/%Y"]
